using System.Collections;
using System.Collections.Generic;
using System.IO;
using UnityEngine;
using UnityEngine.Video;
using UnityEngine.SceneManagement;

namespace UnityClient.GUI.Scenes.Menu
{
    /// <summary>
    /// Prologue/epilogue video player corresponding to CPrologEpilogVideo in VCMI.
    /// Plays intro video sequence, then transitions to MainMenuScene.
    /// </summary>
    public class PrologEpilogScene : MonoBehaviour
    {
        private VideoPlayer videoPlayer = null;
        private int playerStatus = 0;

        void Start()
        {
            GameObject camera = GameObject.Find("MainCamera");
            videoPlayer = camera.GetComponent<VideoPlayer>();
            videoPlayer.playOnAwake = false;
            videoPlayer.loopPointReached += EndReached;

            playerStatus = 0;
            PlayVideo("Videos/Game/3dologo.mp4");
        }

        void UpdateStatus()
        {
            if (playerStatus == 0)
            {
                playerStatus = 1;
                PlayVideo("Videos/Game/nwclogo.mp4");
                return;
            }

            if (playerStatus == 1)
            {
                playerStatus = 2;
                PlayVideo("Videos/Game/h3x1intr.mp4");
                return;
            }

            if (playerStatus == 2)
            {
                SceneManager.LoadScene("MainMenuScene");
            }
        }

        void PlayVideo(string videoFileName)
        {
            videoPlayer.Stop();
            videoPlayer.url = Path.Combine(Application.streamingAssetsPath, videoFileName);
            videoPlayer.frame = 0;
            videoPlayer.isLooping = false;
            videoPlayer.Play();
        }

        void Update()
        {
            if (Input.GetMouseButtonDown(0))
            {
                UpdateStatus();
            }
        }

        void EndReached(VideoPlayer vp)
        {
            UpdateStatus();
        }
    }
}
